...
public class StudentManagement{
    ...
    private static void sortStudentsAscending() {
        ...
        while (sortChoice != 5); //Loop until user enter a number different from 4
    }
    ...
    private static void displayStudentList() {
        System.out.println("----------------------------------------------------------------- SHOW ALL FOUND STUDENTS -------------------------------------------------------------------+");
    ...
    public static void addNewStudent() {
        ...
        Student newStudent = new Student(fullName, studentID, dateOfBirth, email, phoneNumber, scores, 0.0); // set GPA as 0.0 initially
    ...
    public static void writeStudentsToFile(String filename) {
        ...
        //Using BufferedWriter to write data to the file because it is faster and more efficient than FileWriter
    ...
    public static void readStudentsFromFile(String filename) {
        try (BufferedReader reader = new BufferedReader(new FileReader(filename))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                // Assuming the fields are in the order: name, id, dateOfBirth, email, phoneNumber, GPA, subjectScores
                String name = fields[0];
                String id = fields[1];
                String dateOfBirth = fields[2];
                String email = fields[3];
                String phoneNumber = fields[4];
                // Assuming the subject scores are in the order: MAD, OSG, NWC, PRO, SSG, and the scores are separated by a comma (read inputStudent.txt to see the format)
                // Convert the array of scores to a List of Doubles and add the scores to the subjectScores List
                List<Double> subjectScores = Arrays.stream(Arrays.copyOfRange(fields, 5, fields.length)).map(Double::parseDouble).collect(Collectors.toList());
